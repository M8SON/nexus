# Nexus Session Activation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Wire nexus into Claude Code sessions under `~/linux/` so policies load automatically and local-doc snippets are injected at session start.

**Architecture:** A workspace-root `CLAUDE.md` at `~/linux/CLAUDE.md` points at nexus's adapter file (which already references `core.md` + `continuity.md`). A SessionStart hook script at `~/.claude/hooks/nexus-session-start.sh` runs `nexus.cli context --repo-path "$CLAUDE_PROJECT_DIR"` and prints stdout, which Claude Code injects as additional session context. The hook is registered in `~/.claude/settings.json` under `hooks.SessionStart` with the `"startup"` matcher. The script is a silent no-op outside `~/linux/` and on any error.

**Tech Stack:** bash, JSON edit, plain markdown. No new code in the nexus package itself.

**Spec:** `docs/superpowers/specs/2026-04-30-nexus-session-activation-design.md`
...
- **new:** `~/.claude/hooks/nexus-session-start.sh` — SessionStart hook wrapper (executable bash).
- **edit:** `~/.claude/settings.json` — add `hooks.SessionStart` entry that calls the wrapper.

No changes to the nexus Python package. No new tests in `nexus/tests/` — the existing `test_cli.py` and `test_context.py` already cover the CLI subcommand the hook calls.

The validation is manual: start three real Claude Code sessions (one inside `~/linux/nexus`, one inside `~/linux/miniclaw`, one outside `~/linux/`) and confirm the hook fires correctly in each case.
...
**Files:**
- Create: `/home/daedalus/.claude/hooks/nexus-session-start.sh` (mode `0755`)
...
`/home/daedalus/.claude/hooks/nexus-session-start.sh`:
...
```bash
chmod 0755 /home/daedalus/.claude/hooks/nexus-session-start.sh
```

- [ ] **Step 4: Smoke-test the script directly with a real project dir**

```bash
CLAUDE_PROJECT_DIR=/home/daedalus/linux/nexus \
    /home/daedalus/.claude/hooks/nexus-session-start.sh
```

Expected: a multi-line `Project docs:` block listing nexus's own docs (e.g. `WORKING_MEMORY.md`, `README.md`, the spec/plan files under `docs/superpowers/`). Exit code `0`.
...
```bash
( cd /home/daedalus/linux/nexus && unset CLAUDE_PROJECT_DIR && \
    /home/daedalus/.claude/hooks/nexus-session-start.sh )
```
...
- [ ] **Step 7: Smoke-test the failure path (nexus venv missing)**

```bash
( CLAUDE_PROJECT_DIR=/home/daedalus/linux/nexus \
    PATH=/usr/bin:/bin \
    bash -c 'NEXUS_BACKUP=/home/daedalus/linux/nexus/.venv && \
             mv "$NEXUS_BACKUP" "${NEXUS_BACKUP}.tmp" && \
             /home/daedalus/.claude/hooks/nexus-session-start.sh; \
             rc=$?; \
             mv "${NEXUS_BACKUP}.tmp" "$NEXUS_BACKUP"; \
             echo "exit=$rc"' )
```

Expected: no stdout, `exit=0`. Confirms the `[ -x "$NEXUS_DIR/.venv/bin/python" ] || exit 0` guard.
...
The `"startup"` matcher fires only on fresh session start (not on `resume`, `clear`, or `compact`). This keeps token cost predictable: nexus context is injected once per real session, not on every `/clear`.
...
- [ ] **Step 1: Start a fresh Claude Code session inside `~/linux/nexus`**

In a terminal:

```bash
cd /home/daedalus/linux/nexus && claude
```

Expected: when the session opens, the conversation transcript should show a SessionStart hook context block containing `Project docs:` with one line per discovered doc (nexus's `README.md`, `WORKING_MEMORY.md`, the spec and plan markdown files under `docs/superpowers/`, etc.). The agent should also note that it's working in a nexus-managed workspace (because `~/linux/CLAUDE.md` was loaded via the ancestor walk).
...
Expected: NO nexus SessionStart block. The agent should behave exactly as it did before this work landed.
...
Expected: the agent should mention the Karpathy core baseline and the continuity policy (or at least confirm it's working under nexus). This proves the ancestor walk found `~/linux/CLAUDE.md`, which led the agent to read the adapter file, which referenced both policies.
...
| SessionStart hook script at `~/.claude/hooks/nexus-session-start.sh`| 2        |
| Hook uses `nexus.cli context --repo-path "$CLAUDE_PROJECT_DIR"`     | 2 (Step 2) |
...
| Validation case 1: session in `~/linux/nexus`                       | 4 (Step 1) |
...
**Type / signature consistency:** No types or method signatures in this plan — it's all configuration files and shell. The single cross-reference is the path `/home/daedalus/.claude/hooks/nexus-session-start.sh`, which appears identically in Tasks 2 and 3.
